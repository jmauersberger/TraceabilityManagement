				...
				if (_notEquals) {
					_builder.append(".");
					{
						for (final EObject e : secondElements) {
							_builder.append("|");
							String _identifier = UIStringUtil.createUIString(e);
					...
					_builder.newLineIfNotEmpty();
					{
						for (final EObject first : firstElements) {
							String _identifier_1 = UIStringUtil.createUIString(first);
							...
							_builder.append(" ");
							{
								for (final EObject second : secondElements) {
									_builder.append("|");
									{
										boolean _isThereATraceBetween = !traceAdapter
												.getTracesBetween(first, second, traceModel).isEmpty();
